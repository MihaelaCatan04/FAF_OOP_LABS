...
class UniverseClassifier {
    ...
    private fun checkRequirements(creature: Creature, expectedIsHumanoid: Boolean, expectedPlanet: Planet, expectedAge: Int, trait1: Traits, trait2: Traits):Boolean{
    ...
    fun classify(creatures: List<Creature>) {
        for (creature in creatures) {
            when {
                ...
                checkRequirements(creature, false, Planet.Kashyyyk, 400, Traits.HAIRY, Traits.TALL) || checkRequirements(creature, false, Planet.Endor, 60, Traits.SHORT, Traits.HAIRY) -> universes[0].individuals.add(creature)
                checkRequirements(creature, true, Planet.Asgard, 5000, Traits.BLONDE, Traits.TALL) -> universes[1].individuals.add(creature)
                checkRequirements(creature, true, Planet.Betelgeuse, 100, Traits.EXTRA_ARMS, Traits.EXTRA_HEAD) || checkRequirements(creature, false, Planet.Vogsphere, 200, Traits.GREEN, Traits.BULKY) -> universes[2].individuals.add(creature)
                checkRequirements(creature, true, Planet.Earth, Int.MAX_VALUE, Traits.BLONDE, Traits.POINTY_EARS) || checkRequirements(creature, true, Planet.Earth, 200, Traits.SHORT, Traits.BULKY) -> universes[3].individuals.add(creature)
